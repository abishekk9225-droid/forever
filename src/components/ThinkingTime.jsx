...
export default function ThinkingTime({ onComplete, onRestart }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[550px] p-6 text-center z-30 w-full max-w-md mx-auto">
      <AnimatePresence mode="wait">
        {!isTimesUp ? (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="w-full p-8 rounded-3xl bg-zinc-950/90 border border-rose-500/30 shadow-[0_0_50px_rgba(244,114,182,0.3)] space-y-6 backdrop-blur-xl animate-glow-pulse"
          >
            ...
            <h3 className="text-lg sm:text-xl font-serif text-white leading-relaxed">
            ...
            {hasSpecialPerson === null ? (
              <div className="flex gap-4 justify-center pt-2">
                <button
                  onClick={handleYes}
                  className="px-8 py-3 rounded-2xl bg-rose-500 text-white font-medium hover:bg-rose-600 transition shadow-[0_0_20px_rgba(244,63,94,0.4)] cursor-pointer hover:scale-105 active:scale-95"
                >
                  Yes 🙈
                </button>
                <button
                  onClick={handleNo}
                  className="px-8 py-3 rounded-2xl bg-zinc-800 text-zinc-300 font-medium hover:bg-zinc-700 transition cursor-pointer hover:scale-105 active:scale-95"
                >
                  No 🤍
                </button>
              </div>
